...
def change_grammems(grammems, pos, tags_dict, ud_pos):
    ...
    for g in grammems:
        g = correct_gram(g)
        ud_list.append(tags_dict[g.strip()])
...
def RNCtoUD_one_text(text, sym_list, cconj_list, tags_dict):
    ...
    # os.makedirs(os.path.dirname(output_path + conllu_name), exist_ok=True)
    # with open(output_path + conllu_name, 'w', encoding='utf-8', newline='') as f:
    #     f.write(new_text)
    return new_text



# RNCtoUD_one_text(text, sym_list, cconj_list, tags_dict)


# UPDATE 2:
# new CCONJ list +++
# Degree=Pos +++
# Sort with lower(): NumForm=Digit|Number +++
# NounType -> NameType +++
# Sort xpos S.famn.m.anim.sg.nom (except S) +++
# Join sents by ":" (except p class...) +++
